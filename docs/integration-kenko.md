...
このチェック(`companies.hm_enabled`)は、**健康管理Web側の表示も制御**する。

| 画面 | チェックあり | チェックなし |
|---|---|---|
| ストレスチェックWeb(実施者・事務従事者) | 「🩺 健康管理Web」ボタンが出る | 何も出ない |
| 健康管理Web 産業医事務所ダッシュボードの企業一覧 | 表示される | 表示されない |
| 健康管理Web 企業ダッシュボード(`/company`) | 通常どおり | 「ご利用いただけません」 |

企業台帳(`companies`)は両アプリで共有しているため、**企業の登録はストレスチェックWebの
「企業管理」に一本化**されている。健康管理Webにだけ企業を追加することはできない。

---

## 4-3. つまずいた点(2026-09-07)

**症状**: 健康管理Webの企業一覧に、ストレスチェックWebの企業が出てこない
(開発用DBのテストデータだけが表示される)。

**原因**: `NEXT_PUBLIC_` で始まる環境変数は**ビルド時にコードへ埋め込まれる**ため、
Vercelの環境変数を正しい値に直しても、古い値で作られたビルドが動いている間は
以前の接続先(開発用のSupabaseプロジェクト)を見続ける。

**対処**: Vercel → Deployments → 最新の「⋯」→ Redeploy で、
**「Use existing Build Cache」のチェックを外して**再デプロイする。

**接続先の確かめ方**: Supabase の Authentication → Users で対象アカウントの
「Last sign in」を控え、健康管理Webでログアウト→再ログインする。日時が更新されれば
そのプロジェクトに接続できている。
...
- 実施事務従事者(`jimu`)は健康管理Webの対象外です(健康管理Web側で「ご利用いただけません」の
  画面になります)。健診結果や面談記録まで見える必要がないためで、意図した仕様です。
  衛生管理者の方に自社の情報を見ていただく場合は、「企業」役割のアカウントを発行してください。
