...
## 4-4. 事業者担当者を兼ねる場合(実施事務従事者・従業員)

中小企業では、ストレスチェックの実施事務従事者や従業員が、衛生管理・人事の
ご担当者(事業者担当者)を兼ねることが多い。アカウントを2つ持たずに済むよう、
**1つのアカウントに「健康管理Webでは事業者担当者として扱う」印を付ける**方式にした。

| どこ | 何が起きるか |
|---|---|
| ストレスチェックWeb | **これまでどおり**。実施事務従事者は実施事務従事者、従業員は従業員のまま |
| 健康管理Web | 事業者担当者として自社の画面(`/company`)を利用できる |

- 設定は **ユーザー管理 → メンバー一覧**の「事業者担当者を兼ねる」チェックボックス
  (健康管理Web併用の企業でのみ表示)
- 付与・解除できるのは**実施者のみ**。列への直接の更新権限は与えず、
  `set_hm_company_access()` 経由に限定している(本人が自分に付与できない)
- 操作は `hm_company_access_changed` としてアクセスログに残る
...
> 健康管理Webには渡していない(4章のとおり、渡すのは高ストレス者一覧と集計のみ・実施者限定)。
