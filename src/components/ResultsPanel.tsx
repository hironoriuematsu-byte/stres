...
export function ResultsPanel({
  ...
  // 再受検対応: 当年度かつ本人の直近の結果のみ削除可(制約はDB関数側で強制)
  ...
  return (
    <Card>
      ...
      <p style={{ fontSize: 12, color: "#8A9694", marginTop: 12, lineHeight: 1.7 }}>
        行をクリックすると詳細を表示します(詳細の閲覧はアクセスログに記録されます)。結果は5年間保存され、原則削除・改変はできません。例外として、本人から誤回答による再受検の申出があった場合のみ、当年度かつ直近の結果を「再受検のため削除」できます(操作はログに記録されます)。
      </p>
    </Card>
  );
}
