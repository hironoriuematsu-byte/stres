...
// 点と番号を1組で描く。番号は点から少し離した位置に置き、
// どの点の番号かが必ず分かるように細い引き出し線で結ぶ。
function PlotShape(props: {
  cx?: number;
  cy?: number;
  fill?: string;
  payload?: PlotPoint;
  ...
  return (
    <g>
      <line
        ...
        strokeWidth={1}
      />
      <circle cx={cx} cy={cy} r={R} fill={fill} stroke="#fff" strokeWidth={1} />
      <text
        x={lx}
        y={ly}
        textAnchor="middle"
        ...
        fontSize={12}
        fontWeight={700}
        fill={payload.isRef ? "#5B6B6A" : "#22333B"}
        stroke="#fff"
        strokeWidth={3}
        paintOrder="stroke"
      >
...
function JudgeScatter({
  ...
  const spanX = x1 - x0;
  const spanY = y1 - y0;
  // 全国平均(⓪)も同じ配置計算に含め、部署の番号とぶつからないようにする
  const refAsPoints: PlotPoint[] = refPoints.map((r) => ({
    x: r.x,
    y: r.y,
    label: r.label,
    worstRisk: null,
  ...
  const laidOut = assignLabelSide([...refAsPoints, ...data], spanX, spanY);
  const refPts = laidOut.filter((p) => p.isRef);
  const points = laidOut.filter((p) => !p.isRef);
      ...
      <div style={{ width: "100%", height: 300 }}>
        <ResponsiveContainer>
          <ScatterChart margin={{ top: 16, right: 24, bottom: 14, left: 0 }}>
            ...
            />
            {/* 全国平均の位置(男女計・⓪) */}
            <Scatter data={refPts} fill="#5B6B6A" isAnimationActive={false} legendType="none" shape={PlotShape} />
            {/* 番号は点から引き出し線でつなぎ、どの点の番号かが分かるようにする */}
            <Scatter data={points} fill={brand.teal} isAnimationActive={false} shape={PlotShape}>
              {points.map((d, i) => (
        ...
        {groups.length === 0 ? (
          ...
          <>
            ...
            <p style={{ fontSize: 11, color: "#8A9694", margin: "2px 0 0" }}>
              ...
              番号は重ならない向きに置き、細い線でどの点のものかを示しています。
                      ...
                      <React.Fragment key={cat}>
                        ...
                        {SCALES.filter((s) => s.category === cat).map((s) => (
                          <tr key={s.key} style={{ borderBottom: `1px solid ${brand.line}` }}>
                            ...
                            {/* 4段階の単一項目尺度は全国平均が無いため数値を出さない(2.5は参考基準として下の注記で説明) */}
            ...
            <div style={{ fontSize: 10.5, color: "#8A9694", margin: "6px 0 0", lineHeight: 1.8 }}>
              <p style={{ margin: 0 }}>
                ...
                単一項目の4段階尺度(自覚的な身体的負担度・職場環境・技能の活用度・仕事の適性度・働きがい。全国平均欄が「—」の尺度)は
                全国調査に基づく平均値がないため数値を示していません。目安としては目盛り中央の<strong>2.5を参考基準</strong>とみなしてください(参考程度の値です)。
