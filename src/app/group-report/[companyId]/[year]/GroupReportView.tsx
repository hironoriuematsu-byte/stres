...
//
// 基準線(全国平均水準)を全軸で3に統一するため、5段階の多項目尺度のみを
// 描画対象とする。5段階尺度の素点換算表は全国分布で
// 10%/23.3%/33.3%/23.3%/10% となるよう設計されており、評価点の期待値は3。
// 単一項目の4段階尺度(身体的負担度・職場環境・技能の活用度・適性度・
// 働きがい)は回答をそのまま評価点とする方式で全国平均を3に基準化できない
// ため、チャートには含めず平均評価点の表でのみ示す。
...
function radarDataFor(
  ...
  return SCALES.filter((s) => s.category === cat && s.male.length === 5).map((s) => {
    const disp = (v: number | null) => {
      if (v == null) return null;
      const outer = s.direction === "negative" ? 6 - v : v; // 外側ほど良好に反転
      return Math.round(outer * 100) / 100;
    };
        ...
        {groups.length === 0 ? (
          ...
          <>
            ...
            <p style={{ fontSize: 11, color: "#8A9694", margin: "2px 0 0", lineHeight: 1.7 }}>
              ...
              チャートには<strong>全国平均水準が評価点3となる5段階の多項目尺度のみ</strong>を表示しています
              (素点換算表は全国分布を10%/23.3%/33.3%/23.3%/10%に区分する設計のため、5段階尺度の評価点の期待値は3です)。
              単一項目の4段階尺度(自覚的な身体的負担度・職場環境・技能の活用度・仕事の適性度・働きがい)は、
              回答をそのまま評価点とする方式で全国平均を3に基準化できないため、チャートには含めず下の平均評価点の表でご確認ください。
