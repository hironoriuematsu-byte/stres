...
**実施事務従事者(jimu)も自社の従業員を招待できます**(jimuダッシュボードの「従業員招待」タブ。単発またはメールアドレス1列のCSV一括)。jimuが招待できるのは自社・従業員ロールのみで、他社への招待やjimu/officeロールの発行はAPI側で拒否されます。

### 従業員の登録方法②: 配布URL・QRコード(推奨・一斉配布向け)

個別招待の代わりに、**受検用の配布URL**で従業員が自己登録する方式が使えます:

1. officeダッシュボードの「配布URL・QR」タブで、企業・実施年度を選んで**発行**
2. jimuは自分のダッシュボードの「配布URL・QR」タブでURLをコピー/QRをダウンロードし、**社内一斉メールや掲示で配布**
3. 従業員はURL(`/join/<トークン>`)を開くと会社名・実施年度が表示され、自分のメールアドレス+パスワードで登録(登録したメールアドレスはログインに必要)
4. **確認メールのリンクをクリックして本人認証**(メール所有確認)が完了すると受検に進める

- URLのトークンが企業・年度を一意に決めるため、他社従業員の混入はありません
- officeはURLの**停止/再開/再発行**(旧URL無効化)ができ、実施期間の制御に使えます
- 有効な配布URLがある間は、受検はその**実施年度**で記録されます(未発行時は現在年度)
